...
<div class="container">
      ...
      <p class="movie-full__info"><b>Genres</b>: <span v-for="genre in movie.genres" v-bind:key="genre.id">{{genre.name}} </span></p>
    ...
    <Similar 
      v-bind:similar="getSimilarMovies"
    />
    <!-- {{movie}} -->
...
export default {
  ...
  async mounted() {
    const res = await fetch(`https://api.themoviedb.org/3/movie/${this.$route.params.id}?api_key=60e1831dec35a216fdaff508cdf5675c&language=en-US`);
  ...
  computed: {
    ...
    image() {
      ...
      return config.images.secure_base_url + config.images.backdrop_sizes[0] + this.movie.poster_path;
    },
